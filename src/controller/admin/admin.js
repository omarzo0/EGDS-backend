...
const updateAdmin = async (req, res) => {
    try {
        ...
        if (req.user.role !== "superadmin" && req.user.id !== id) {
            return res.status(403).json({ message: "Unauthorized to update this admin" });
        }
...
module.exports = {
    getAdminList: [verifyToken, isAdmin, getAdminList],
    createAdmin: [verifyToken, isAdmin, createAdmin],
    updateAdmin: [verifyToken, isAdmin, updateAdmin],
    deleteAdmin: [verifyToken, isAdmin, deleteAdmin],
};